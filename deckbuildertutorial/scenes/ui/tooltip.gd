...
func show_tooltip(icon:Texture, text:String) -> void:
	...
	tween.tween_property(self, "modulate", Color.WHITE, fade_secondes)
...
func hide_tooltip() -> void:
	...
	get_tree().create_timer(fade_secondes, false).timeout.connect(hide_animation)
...
func hide_animation() -> void:
	if (is_visible):
		return
	tween = create_tween().set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_CUBIC)
	tween.tween_property(self, "modulate", Color.TRANSPARENT, fade_secondes)
